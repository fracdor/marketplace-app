...
**Two predictions from reading the migrations directly, NOW CONFIRMED against the actual generated `lib/database.types.ts` (Task 1 landed, commit `d60bddf`) — no more guessing needed for these two:**

1. **CONFIRMED TRUE**: `tasks.status` and `offers.status` (`text not null ... check (status in (...))`, `supabase/migrations/20260702000003_create_tasks.sql:11`, `20260702000004_create_offers.sql:8`) generate as plain `status: string` in `lib/database.types.ts` (lines 65, 154), not `TaskStatus`/`OfferStatus`. **Required fix, Task 5**: override just that field back to the literal union: `Omit<Database['public']['Tables']['tasks']['Row'], 'status'> & { status: TaskStatus }` (same pattern for `Offer`/`OfferStatus`). This preserves the exhaustive-switch-without-`default` compile-time guarantee documented in `CLAUDE.md` — the DB already enforces the invariant via CHECK + the `enforce_task_status_transitions` trigger, so telling TypeScript about it is a legitimate narrowing, not a false claim.
2. **CONFIRMED FALSE — do NOT add a cast for this**: `tasks.budget_reference`/`offers.price` (`numeric(12,2)`) generate as `budget_reference: number | null` and `price: number` in `lib/database.types.ts` (lines 64, 73, 147) — this CLI version (`supabase@2.109.1`) maps `numeric` to `number`, not `string`. The original hypothesis (some versions map numeric→string) does NOT apply here. No cast-back-to-number workaround needed anywhere — if you find yourself wanting to add one, stop, you're solving a problem that doesn't exist in this schema.
