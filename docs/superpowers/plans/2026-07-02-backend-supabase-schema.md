...
- [x] **Step 3: Create the extensions migration**
...
- [x] **Step 1: Write the helper file and confirm it doesn't exist yet**
...
- **`task_id references public.tasks(id) on delete cascade` silently destroys offers, including pending ones, when a client deletes their task.** Found during the final whole-schema review (Task 8): `tasks_delete_own_open` lets a client delete any of their own `open` tasks, and the cascade means any offers already submitted on it vanish with no trace and no notification to the freelancers who submitted them. This is an accepted MVP limitation, not a bug fix for this plan — if it needs to change later (offers representing a real commitment worth preserving), the fix is `on delete restrict` on this FK (forcing the app to block/warn before deleting a task with non-withdrawn offers) rather than a schema rewrite.
...
- **Concurrency correctness (two simultaneous `accept_offer()` calls on the same task) is manually verified, not covered by an automated regression test.** The design spec calls for this explicitly. pgTAP runs single-connection/sequential, so it can't express two genuinely concurrent transactions without an external two-connection harness (e.g. two `psql` sessions, one blocked on the other's `for update` lock). The row-locking reasoning was independently verified live during review (a second call blocks on the `tasks` row lock, then re-reads `status <> 'open'` once unblocked and raises the expected exception) but that check is not repeatable as-is — it's not part of `npx supabase test db`. If this needs to be a durable regression rather than a one-time manual check, it requires a small script outside pgTAP, not a plan-doc fix.
...
- [x] **Step 1: Reset the database from scratch and run the entire suite**
...
- [x] **Step 2: Confirm the RLS gap check**
...
- [x] **Step 3: Commit the final state (if anything changed)**
