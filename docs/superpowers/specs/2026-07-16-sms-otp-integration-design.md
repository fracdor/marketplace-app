...
- `verify-phone.tsx`: existing tests updated for the removed dev-code hint and for the code-verification catch block now using `mapAuthError` instead of the raw message. `PhoneForm.tsx`'s own tests need no change (its `mapAuthError` call site is unchanged) but should gain a case confirming a thrown `'invalid_phone'` error surfaces "Número de celular inválido." end-to-end.
